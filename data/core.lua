--=====================================================================================
-- PLU | PokemonLevelUp - core.lua
-- Version: 3.0.0
-- Author: DonnieDice
-- RGX Mods Collection - RealmGX Community Project
--=====================================================================================

local RGX = assert(_G.RGXFramework, "PLU: RGX-Framework not loaded")

PLU = PLU or {}

local ADDON_VERSION = "3.0.0"
local ADDON_NAME = "PokemonLevelUp"
local ICON_PATH = "|Tinterface/addons/PokemonLevelUp/media/icon:16:16|t"
local PREFIX = ICON_PATH .. " |cffff0000PLU:|r"
...
handle:SetLocale(PLU.L)
L = PLU.L or {}
handle:Init()
initialized = true
end, "PLU_ADDON_LOADED")

RGX:RegisterEvent("PLAYER_LEVEL_UP", function()
if initialized then
handle:Play()
end
end, "PLU_PLAYER_LEVEL_UP")
...
end, "PLU_PLAYER_LOGIN")

RGX:RegisterEvent("PLAYER_LOGOUT", function()
handle:Logout()
end, "PLU_PLAYER_LOGOUT")

RGX:RegisterSlashCommand("plu", function(msg)
local ok, err = pcall(HandleSlashCommand, msg)
if not ok then
print(PREFIX .. " |cffff0000PLU Error:|r " .. tostring(err))
end
end, "PLU_SLASH")
